  ...
  <div class="questionBox-container">
    ...
    <div v-if="isCompleted">
      <b-jumbotron>
        ...
        <template v-slot:lead>
          you answered {{ correctAns }} questions right from 10 geography
          questions.
        </template>
        <hr class="my-4" />
        <p v-if="correctAns >= 0 && correctAns < 4">you performed poorly</p>
        <p v-if="correctAns >= 4 && correctAns <= 6">you can do better</p>
        <p v-if="correctAns > 6 && correctAns < 8">
          you have performance was satisfactory
        </p>
        <p v-if="correctAns >= 8 && correctAns <= 9">
          your performance was good
        </p>
        <p v-if="correctAns == 10">your performance was excellent</p>
        <b-container>
          <b-button class="m-2" variant="primary" @click="handleRouteChange">
            select new category
          </b-button>
          <b-button class="m-2" variant="danger">Retake Quiz</b-button>
        </b-container>
      </b-jumbotron>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    suffleOptionsFunction() {
      ...
      this.correctIndex = this.suffleOptions.indexOf(
        this.currentQuestion.correct_answer
      );
    },
    checkSubmittedAns() {
